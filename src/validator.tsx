...
const validator = (
  ...
  rules.forEach(r => {
    ...
    if (isEmpty(value) && r.required) {
      addErrors(r.key, `${r.label} is required`);
    }
    if (!isEmpty(value) && value.length < r.minLength!) {
      addErrors(r.key, `${r.label} have to be longer than {r.minLength!}`);
    }
    if (!isEmpty(value) && value.length > r.maxLength!) {
      addErrors(r.key, `${r.label}have to be shorter than ${r.maxLength!}`);
    }
    if (!isEmpty(value) && parseInt(value, 10) < r.minValue!) {
      addErrors(r.key, `${r.label} is bigger than {r.minValue!}`);
    }
    if (!isEmpty(value) && parseInt(value, 10) > r.maxValue!) {
      addErrors(r.key, `${r.label} is smaller than{r.maxValue!}`);
    }
    if (!isEmpty(value) && r.pattern && !r.pattern.test(value)) {
      addErrors(r.key, `${r.label}has wrong synthax`);
    }
  });
